...
TEST_TEMPLATE.each do |key, value|
  if !Question.find_by_test_question(key)
    Question.create(
      test_question: key,
      option_one: TEST_TEMPLATE[key][0],
      option_two: TEST_TEMPLATE[key][1],
      option_three: TEST_TEMPLATE[key][2],
      option_four: TEST_TEMPLATE[key][3]
    )
  end
end